...
1. **路径解析**
   ...
   同时派生 JdocMunch 索引名 `<jdocmunch_repo>`：取 `workspace_root` 目录名，转小写，将空格与非字母数字合并为 `-`，去首尾 `-`，再追加 `-wiki`。禁止在 skill 中写死某个 vault 名或绝对路径。
...
2. **类型判定与重复防护**
   ...
   - 现有 tag 池覆盖不全时才新增 tag。新增 tag 必须 kebab-case，并先用脚本确认规范化建议，避免长主题 tag 出现拼写错误：
     ```bash
...
3. **提炼与写入**
   ...
   - tags：从现有 tag 池优先继承；只以 kebab-case 写主题语义；可用 `topic-slug`、`method-name` 这类抽象示例理解格式，但不得把示例当作默认 tag。
   - PDF 深读失败：保留 source 骨架并标注 `需要人工补读`，不归档。

4. **同步 index、归档、刷新检索索引并写日志**
   ```bash
   python ".agents/scripts/write_index.py" --index-path "<index_path>" --section "Sources" --page "摘要-foo" --description "一句话摘要"
   ```
   仅当 source/entity/concept 与 `index.md` 全部就绪且 PDF 深读未失败（无 PDF 提取失败）时归档。归档成功后，把 source/entity/concept frontmatter 中旧 `raw/02-papers/` 的 `sources:` 更新为 `raw/09-archived/`；`wiki/` 不得残留 `raw/02-papers/` 的 `sources:` 引用。
   若写入了 tag 池外的 tag，运行如下脚本：
   ```bash
   python ".agents/scripts/wiki_tags.py" --wiki-dir "<wiki_dir>" --index-path "<index_path>" --update-index
   ```

   完成所有 wiki 写入、`index.md` 同步、归档与 frontmatter `sources:` 回写后，刷新 JdocMunch 索引，让后续 `/query` 能直接命中新页面。普通 ingest 只新增或修改少量页面，默认使用增量索引：
   ...
   只有在索引明显失真、section 检索连续空命中、路径结构大改、或怀疑索引漂移时，才改用 `incremental=False` 做一次全量重建。JdocMunch 刷新失败时，不要回滚已完成的 wiki 写入；记录失败原因，并在最终报告中说明“检索索引未刷新，需后续重建”。

   最后写入 append-only 日志；日志 detail 应记录 JdocMunch 刷新状态（成功 / 失败原因 / 是否全量重建）：
   ```bash
   python ".agents/scripts/write_log.py" --log-path "<log_path>" --action ingest --summary "<操作简述>" --detail "变更=..." --detail "冲突=无" --detail "JdocMunch=incremental 成功"
   ```
...
论文 source 用以下区块：`## Metadata`、`## 核心摘要`、`## 关键图示`、`## 表格速览`、`## 关键公式`、`## 代码对照线索（如有公开或本地仓库，否则跳过）`、`## 关联连接`。
...
- [ ] ingest 完成后已用 `incremental=True` 刷新 JdocMunch；若改用 `incremental=False`，已说明原因（索引失真/结构大改/漂移修复）。
